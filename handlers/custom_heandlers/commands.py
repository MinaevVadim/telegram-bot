...
def start_command(message: Message) -> None:
    """
    Пользователь вводит соответствующую команду,
    после этого создается объект класса User, записываем в словарь
    команду и id чата. Затем открываем базу данных
    и заполням таблицу нужными данными для команды history
    """
    ...
    with db:
        dct_history[message.chat.id] = InfoHistory.create(
            id_name=User.dct[message.chat.id].id,
            command=message.text, date=datetime.datetime.now()
        )
    bot.send_message(
        message.from_user.id,
        "Введите пожалуйста город где будет проводиться поиск"
    )
...
def get_city(message: Message) -> None:
    """
    Здесь мы ожидаем от пользователя ввод нужного города,
    делаем запрос к API, если сообщение было корректное, то
    просим его уточтить местоположение отеля в городе с
    помощью онлайн кнопок, в противном случае
    просим ввести город повторно
    """
    search_cities = SearchObject(
    ...
    if api is None:
        bot.send_message(
            message.from_user.id,
            'Не корректно введены данные, ничего не удалось найти, попробуйте еще раз&#10071',
            parse_mode='HTML'
        )
        bot.register_next_step_handler(message, get_city)
    else:
...
def price_range(message: Message) -> None:
    """
    Тут ожидаем от пользователя желаемый диапазон цен за отель,
    делаем проверку направильность ввода, обязательно через пробел
    и текст должен содержать только цифры, данные записываем в словарь
    """
    message_lst = [i.isdigit() for i in message.text.split()]
    text_split = message.text.split()
    text_split_0 = message.text.split()[0]
    text_split_1 = message.text.split()[1]
    if all(message_lst) and len(text_split) == 2 and int(text_split_0) < int(text_split_1):
        User.dct[message.chat.id].price = message.text.split()
        bot.send_message(
            message.from_user.id,
            "Введите пожалуйста желаемый диапазон расстояния от отеля до центра через пробел (км)"
        )
        bot.register_next_step_handler(message, distance_center)
    else:
        bot.send_message(
            message.from_user.id,
            'Пожалуйста введите только цифрами и через пробел, попробуйте еще раз&#10071',
            parse_mode='HTML'
        )
...
def distance_center(message: Message) -> None:
    """
    Тут ожидаем от пользователя желаемый диапазон расстояния от отеля
    до центра города, делаем проверку на правильность ввода, обязательно
    через пробел и текст должен содержать только цифры, так же записываем
    в словарь диапазон расстояния
    """
    try:
        float(message.text.split()[0]), float(message.text.split()[1])
        if float(message.text.split()[0]) < float(message.text.split()[1]):
            User.dct[message.chat.id].distance = message.text.split()
            bot.send_message(
                message.from_user.id,
                'Сколько отелей вывести (Не более 10)?'
            )
...
def question_about_photo(message: Message) -> None:
    """
    Получаем от пользователя нужное количество отелей для поиска и
    записываем данные в словарь, обрабатываем на коректность данных,
    далее справшиваем в необходимости загрузки фотографий, предлагая
    ответ в ввиде онлайн кнопок
    """
    if all([i.isdigit() for i in message.text]):
        ...
        bot.send_message(
            message.from_user.id,
            text=text,
            reply_markup=keyboard_for_id()
        )
    else:
        bot.send_message(
            message.from_user.id,
            'Пожалуйста введите только цифрами, попробуйте еще раз&#10071',
            parse_mode='HTML'
        )
        bot.register_next_step_handler(message, question_about_photo)
...
def ready_photo(message: Message) -> None:
    """
    Тут получаем информацию о неоходимом количестве фотографий с
    проверкой на корректность и в зависимости от введенной команды
    пользователя выводим весь итоговый результат по поиску отелей
    с фотографиями и записываем в базу данных информацию по по
    каждому отелю для команды history
    """
    if all([i.isdigit() for i in message.text]):
        User.dct[message.chat.id].count_photos = int(message.text)
        bot.send_message(
            message.chat.id,
            'Подождите пожалуйcта, идет обработка результата &#9203',
            parse_mode='HTML'
        )
        search_hotels = SearchObject(
            search_hotel(User.dct[message.chat.id].call_data), config.search_list)
        if User.dct[message.chat.id].command == '/lowprice':
            text_hotels = ''
            for with_photo1 in text_transformation(
                    property_founding(
                        search_hotels.request_to_api(),
            ...
            text_hotels = ''
            for with_photo2 in text_transformation(
                    property_founding(
                        search_hotels.request_to_api(),
                        User.dct[message.chat.id].check_in,
                        User.dct[message.chat.id].check_out,
            ...
            text_hotels = ''
            for with_photo3 in text_transformation(
                    property_founding(
                        search_hotels.request_to_api(),
                        User.dct[message.chat.id].check_in,
                        User.dct[message.chat.id].check_out,
...
def hotels_without_photos(call) -> None:
    """
    В зависимости от введенной команды пользователя выводим всю
    итоговую информацию по отелям без фотографий
    """
    ...
    search_hotels = SearchObject(
        search_hotel(
            User.dct[call.message.chat.id].call_data), config.search_list
    )
    bot.edit_message_reply_markup(call.message.chat.id, call.message.message_id)
    bot.delete_message(call.message.chat.id, call.message.message_id)
    if User.dct[call.message.chat.id].command == '/lowprice':
        text_hotels = ''
        for no_photo1 in text_transformation(
                property_founding(
                    search_hotels.request_to_api(),
        ...
        text_hotels = ''
        for no_photo2 in text_transformation(
                property_founding(
                    search_hotels.request_to_api(),
                    User.dct[call.message.chat.id].check_in,
                    User.dct[call.message.chat.id].check_out,
        ...
        text_hotels = ''
        for no_photo3 in text_transformation(
                property_founding(
                    search_hotels.request_to_api(),
                    User.dct[call.message.chat.id].check_in,
                    User.dct[call.message.chat.id].check_out,
                    ...
                    distance_c=User.dct[call.message.chat.id].distance
                ), False
        ):
...
def question_date(call) -> None:
    """
    В зависимости от того, какую кнопку нажал пользователь, получаем
    id местоположения и записываем его в словарь, далее запрашиваем
    желаемую дату въезда в отель с помошью календаря
    """
    bot.answer_callback_query(callback_query_id=call.id, text='Спасибо за выбор!')
    bot.edit_message_reply_markup(call.message.chat.id, call.message.message_id)
    User.dct[call.message.chat.id].call_data = call.data
    bot.delete_message(call.message.chat.id, call.message.message_id)
    calendar, step = MyCustomCalendar(
        calendar_id='entry',
    ...
    bot.send_message(
        call.message.chat.id,
        f"Выберите желаемую дату въезда",
...
def date_entry(call) -> None:
    """
    Тут происходит взаимодействие с календарем, так же записываем
    в словарь дату въезда из отеля
    """
    result, key, step = MyCustomCalendar(
        calendar_id='entry',
        current_date=date.today(),
        min_date=date.today(),
        max_date=date.today() + timedelta(days=365),
    ...
    if not result and key:
        ...
        bot.send_message(
            call.message.chat.id,
            f"Выберите желаемую дату выезда",
            reply_markup=calendar
        )
...
def departure_date(call) -> None:
    """
    Здесь происходит взаимодействие с календарем, так же записываем в
    словарь дату выезда из отеля и после, в зависимости от команды
    пользователя задаем вопрос для прохождения дальнейшего сценария
    """
    result, key, step = MyCustomCalendar(
        calendar_id='exit',
        current_date=date.today(),
